...
test:
	...
	@docker exec fresh-control-plane bash -c "cp /root/srvs/0-cloud-config.sh /root/ && cd /root && bash 0-cloud-config.sh"
	@docker exec fresh-control-plane bash -c "su - ubuntu -c 'cd srvs && cp .env.example .env && echo \"PERSONAL_SSH_KEY=\\\"ssh-rsa AAAAB3NzaC1yc2EAAAADAQABAAABgQC7test ubuntu@test\\\"\" >> .env && echo \"CONTROL_PLANE_SSH_KEY=\\\"ssh-rsa AAAAB3NzaC1yc2EAAAADAQABAAABgQC7test ubuntu@test\\\"\" >> .env'"
	@docker exec fresh-control-plane bash -c "su - ubuntu -c 'cd srvs && sudo make control-plane'"
	@echo "Testing Worker..."
	@docker exec fresh-worker bash -c "cp /root/srvs/0-cloud-config.sh /root/ && cd /root && bash 0-cloud-config.sh"
	@docker exec fresh-worker bash -c "su - ubuntu -c 'cd srvs && cp .env.example .env && echo \"PERSONAL_SSH_KEY=\\\"ssh-rsa AAAAB3NzaC1yc2EAAAADAQABAAABgQC7test ubuntu@test\\\"\" >> .env && echo \"CONTROL_PLANE_SSH_KEY=\\\"ssh-rsa AAAAB3NzaC1yc2EAAAADAQABAAABgQC7test ubuntu@test\\\"\" >> .env && echo \"CONTROL_PLANE_IP=\\\"172.21.0.10\\\"\" >> .env'"
	@docker exec fresh-worker bash -c "su - ubuntu -c 'cd srvs && sudo make worker'"
